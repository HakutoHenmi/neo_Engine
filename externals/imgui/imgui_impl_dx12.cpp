...
//  2025-05-07: DirectX12: Honor draw_data->FramebufferScale to allow for custom backends and experiment using it (consistently with other renderer backends, even though in normal condition it is not
//  set under Windows). 2025-02-24: DirectX12: Fixed an issue where ImGui_ImplDX12_Init() signature change from 2024-11-15 combined with change from 2025-01-15 made legacy ImGui_ImplDX12_Init() crash.
//  (#8429) 2025-01-15: DirectX12: Texture upload use the command queue provided in ImGui_ImplDX12_InitInfo instead of creating its own. 2024-12-09: DirectX12: Let user specifies the DepthStencilView
//  format by setting ImGui_ImplDX12_InitInfo::DSVFormat. 2024-11-15: DirectX12: *BREAKING CHANGE* Changed ImGui_ImplDX12_Init() signature to take a ImGui_ImplDX12_InitInfo struct. Legacy
//  ImGui_ImplDX12_Init() signature is still supported (will obsolete). 2024-11-15: DirectX12: *BREAKING CHANGE* User is now required to pass function pointers to allocate/free SRV Descriptors. We
//  provide convenience legacy fields to pass a single descriptor, matching the old API, but upcoming features will want multiple. 2024-10-23: DirectX12: Unmap() call specify written range. The range
//  is informational and may be used by debug tools. 2024-10-07: DirectX12: Changed default texture sampler to Clamp instead of Repeat/Wrap. 2024-10-07: DirectX12: Expose selected render state in
//  ImGui_ImplDX12_RenderState, which you can access in 'void* platform_io.Renderer_RenderState' during draw callbacks. 2024-10-07: DirectX12: Compiling with '#define ImTextureID=ImU64' is unnecessary
//  now that dear imgui defaults ImTextureID to u64 instead of void*. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11. 2021-06-29: Reorganized backend to pull data from a
//  single structure to facilitate usage with multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19: DirectX12: Replaced direct access to ImDrawCmd::TextureId with a call to
//  ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: DirectX12: Change blending equation to preserve alpha in output buffer. 2021-01-11: DirectX12: Improve Windows 7 compatibility (for
//  D3D12On7) by loading d3d12.dll dynamically. 2020-09-16: DirectX12: Avoid rendering calls with zero-sized scissor rectangle since it generates a validation layer warning. 2020-09-08: DirectX12:
//  Clarified support for building on 32-bit systems by redefining ImTextureID. 2019-10-18: DirectX12: *BREAKING CHANGE* Added extra ID3D12DescriptorHeap parameter to ImGui_ImplDX12_Init() function.
...
static ImGui_ImplDX12_Data* ImGui_ImplDX12_GetBackendData() { return ImGui::GetCurrentContext() ? (ImGui_ImplDX12_Data*)ImGui::GetIO().BackendRendererUserData : nullptr; }
...
void ImGui_ImplDX12_UpdateTexture(ImTextureData* tex) {
	...
	if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates) {
		...
		if (bd->pTexUploadBuffer == nullptr || upload_size > bd->pTexUploadBufferSize) {
			...
			HRESULT hr = bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&bd->pTexUploadBuffer));
...
bool ImGui_ImplDX12_CreateDeviceObjects() {
	...
	// Create the root signature
	{
		...
		desc.Flags = D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT | D3D12_ROOT_SIGNATURE_FLAG_DENY_HULL_SHADER_ROOT_ACCESS | D3D12_ROOT_SIGNATURE_FLAG_DENY_DOMAIN_SHADER_ROOT_ACCESS |
...
bool ImGui_ImplDX12_Init(
    ID3D12Device* device, int num_frames_in_flight, DXGI_FORMAT rtv_format, ID3D12DescriptorHeap* srv_descriptor_heap, D3D12_CPU_DESCRIPTOR_HANDLE font_srv_cpu_desc_handle,
...
void ImGui_ImplDX12_ShutdownMultiViewportSupport() { ImGui::DestroyPlatformWindows(); }
